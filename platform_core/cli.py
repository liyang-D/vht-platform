...
def get_project(cursor, project: str) -> dict[str, Any] | None:
    cursor.execute(
        """
        SELECT id, name, is_valid, quota, usage_count, created_at, updated_at
        FROM projects
...
def create_project_if_missing(
    ...
    cursor.execute(
        """
        INSERT INTO projects (name, is_valid, quota, usage_count)
        VALUES (%s, TRUE, %s, 0)
        RETURNING id, name, is_valid, quota, usage_count, created_at, updated_at;
        """,
        (project_name, quota),
    )
    return cursor.fetchone()
...
def list_projects(args: argparse.Namespace) -> None:
    ...
    output_rows(
        rows,
        ["id", "name", "is_valid", "quota", "usage_count", "created_at"],
        args.json,
    )
